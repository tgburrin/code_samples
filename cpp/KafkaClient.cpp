...
KafkaClient::KafkaClient(ProcessCfg *cfg) {
    debug = false;
    running = false;
...
void KafkaClient::SetDebug(bool b) {
    debug = b;
}

int64_t KafkaClient::GetSourceReference()
{
    return sourceReference;
}

void KafkaClient::SetSourceReference(int64_t sr)
{
    sourceReference = sr;
}

bool KafkaClient::_ProcessMessage(RdKafka::Message *msg)
{
    ...
    switch (msg->err()) {
        ...
        case RdKafka::ERR_NO_ERROR:
            if ( msg->len() )
            {
                char tmpMsg[msg->len()+1];
                tmpMsg[msg->len()] = (char)'\0';
                strncpy(tmpMsg, (const char *)msg->payload(), msg->len());
                message = string(tmpMsg);
            }
            else
            {
                message.clear();
...
void KafkaClient::Stop() {
    consumer->stop(topic, partition);
}

string *KafkaClient::Read()
{
    string *rv = 0;
